...
(function () {
    ...
    PingPong.GameScene.prototype = {
        ...
        loadModels: function () {
            ...
            function onTableLoad(geometry, materials) {
                ...
                var netCenter = new THREE.Vector3(
                    0,
                    tableSize.height - netHeight / 2,  // 与你当前版本保持一致
                    0
                );
        ...
        update: function () {
            ...
            // 物理模拟：抛球 或 对拉 阶段都要更新球
            if (state === STATES.PLAYING || state === STATES.TOSSING) {
                // 在本帧模拟之前，记录上一帧的 z 速度
                var v = this.simulation.getLinearVelocity();
                if (v) {
                    lastBallVz = v.z;
                }
        ...
        return;
                    ...
                    if (state === STATES.PLAYING) {
                        if (lastBallVz < 0) {
                            // 球原来是从玩家飞向 AI → 玩家不过网 → AI 得分
                            this.awardPoint("ai");
                        } else if (lastBallVz > 0) {
                            // 球原来是从 AI 飞向玩家 → AI 不过网 → 玩家得分
                            this.awardPoint("player");
                        } else {
                            // 极端情况：速度刚好接近 0，当作无效回合，只重置
                            this.resetServe();
                            return;
                        }

                        // 进入 NET_HIT 状态：让软网反弹演 0.3 秒
                        state = STATES.NET_HIT;
                        netHitTimer = NET_HIT_DURATION;
                        return;
                    }
